...
class VendorControllerTest {
        ...
        void givenVendorsExist_whenGetVendorsByStore_thenReturnsVendorList() throws Exception {
                // given
                VendorResponse vendor1 = new VendorResponse(UUID.randomUUID(), "농협마트", "이영희", "010-2222-2222",
                                "v2@test.com", 2, VendorStatus.ACTIVE, OffsetDateTime.now(), OffsetDateTime.now());
                VendorResponse vendor2 = new VendorResponse(UUID.randomUUID(), "신선마트", "김철수", "010-1111-1111",
                                "v1@test.com", 1, VendorStatus.ACTIVE, OffsetDateTime.now(), OffsetDateTime.now());

                CustomUserDetails userDetails = mock(CustomUserDetails.class);
                given(userDetails.getUserId()).willReturn(userId);

                Page<VendorResponse> page = new PageImpl<>(List.of(vendor1, vendor2), PageRequest.of(0, 10), 2);
                PageResponse<VendorResponse> response = PageResponse.from(page);

                given(vendorService.getVendorsByStore(eq(userId), eq(storePublicId), any(VendorSearchRequest.class),
                                any(Pageable.class)))
                                .willReturn(response);
        ...
        void givenMixedStatus_whenGetVendorsByStoreWithActiveFilter_thenReturnsActiveOnly() throws Exception {
                // given
                VendorResponse activeVendor = new VendorResponse(vendorPublicId, "신선마트", "김철수", "010-1111-1111",
                                "v1@test.com", 1, VendorStatus.ACTIVE, OffsetDateTime.now(), OffsetDateTime.now());

                CustomUserDetails userDetails = mock(CustomUserDetails.class);
                given(userDetails.getUserId()).willReturn(userId);

                Page<VendorResponse> page = new PageImpl<>(List.of(activeVendor), PageRequest.of(0, 10), 1);
                PageResponse<VendorResponse> response = PageResponse.from(page);
        ...
        void givenValidRequest_whenUpdateVendor_thenReturnsUpdatedVendor() throws Exception {
                ...
                given(vendorService.updateVendor(eq(storePublicId), eq(vendorPublicId), eq(userId),
                                any(VendorUpdateRequest.class)))
                                .willReturn(response);
