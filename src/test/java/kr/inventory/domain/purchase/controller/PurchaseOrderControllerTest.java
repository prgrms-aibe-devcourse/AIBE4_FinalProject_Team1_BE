...
class PurchaseOrderControllerTest {
    ...
    void givenValidStore_whenGetList_thenReturnsOk() throws Exception {
        ...
        mockMvc.perform(get("/api/purchase-orders/{storePublicId}", storePublicId)
                        .with(user(userDetails)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].purchaseOrderPublicId").value(purchaseOrderPublicId.toString()));
    }
    ...
    void givenValidPurchaseOrder_whenGetDetail_thenReturnsOk() throws Exception {
        ...
        // when & then
        mockMvc.perform(get("/api/purchase-orders/{storePublicId}/{purchaseOrderPublicId}", storePublicId, purchaseOrderPublicId)
                        .with(user(userDetails)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.purchaseOrderPublicId").value(purchaseOrderPublicId.toString()));
    }
    ...
    void givenValidRequest_whenUpdate_thenReturnsOk() throws Exception {
        ...
        given(purchaseOrderService.update(eq(1L), eq(storePublicId), eq(purchaseOrderPublicId), any(PurchaseOrderUpdateRequest.class)))
                .willReturn(response);

        // when & then
        mockMvc.perform(put("/api/purchase-orders/{storePublicId}/{purchaseOrderPublicId}", storePublicId, purchaseOrderPublicId)
                        .with(user(userDetails))
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
    ...
    void givenValidPurchaseOrder_whenCancel_thenReturnsOk() throws Exception {
        ...
        // when & then
        mockMvc.perform(post("/api/purchase-orders/{storePublicId}/{purchaseOrderPublicId}/cancel", storePublicId, purchaseOrderPublicId)
                        .with(user(userDetails))
                        .with(csrf()))
                .andExpect(status().isOk())
    ...
    void givenValidPurchaseOrder_whenDownloadPdf_thenReturnsPdf() throws Exception {
        ...
        // when & then
        mockMvc.perform(get("/api/purchase-orders/{storePublicId}/{purchaseOrderPublicId}/pdf", storePublicId, purchaseOrderPublicId)
                        .with(user(userDetails)))
                .andExpect(status().isOk())
